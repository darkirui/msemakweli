DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spdatimtxrttdata`(
...
BEGIN
    set @querydropformerrecords = concat('
			...
			left join(	
				...
				from(
					select facilityname,personid from patienttxrtt where voided = 0 and reportingperiod = ''',reportingperiod,'''
				)report left join(
