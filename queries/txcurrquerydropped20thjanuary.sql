DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sprefreshpatienttxcurr`(
...
BEGIN
    ...
    DROP TABLE IF EXISTS schemapatienttxcurr;
    create table schemapatienttxcurr (
        databasename varchar(250)
    );
	
    set @querydropformerrecords = concat('update patienttxcurr set voided = 1 WHERE 
    ...
    insert into schemapatienttxcurr
    SELECT databasename from facilitydatabases;
    
	refreshpatienttxcurrloop:LOOP
        set selecteddatabasename = (select databasename from schemapatienttxcurr limit 1);
        set newreportingperioddate = DATE_ADD(reportenddate, INTERVAL 1 DAY);
        set @querypatientvisits = concat('insert into patienttxcurr(facilityname,personid,txcurrstatus,daysdefaulted,
        reportingtimespan,reportingperiod,reportenddate,refreshdate) 
			SELECT
				...
				CASE WHEN activeclients.NextAppointmentDate < ''',newreportingperioddate,''' THEN 
					...
					''',reportingtimespan,''',''',reportingperiod,''',''',reportenddate,''',''',refreshdate,''' AS refreshdate
				FROM(
				SELECT * FROM(
					...
					FROM (
						...
						WHERE concept_id = 5096 and DATE_ADD(value_datetime, INTERVAL 30 DAY) >= ''',reportenddate,''' 
        ...
        delete from schemapatienttxcurr where databasename = selecteddatabasename;
        IF ((select count(*) from schemapatienttxcurr) > 0) THEN
            ITERATE refreshpatienttxcurrloop;
		ELSE 
			DROP TABLE schemapatienttxcurr;
        END IF;
        LEAVE refreshpatienttxcurrloop;

    END LOOP refreshpatienttxcurrloop;
END$$
DELIMITER ;
