DELIMITER $$
drop procedure if exists spdatimtxcurrdata;
...
BEGIN
    set @querydropformerrecords = concat('
			...
			Count(*) As Total,
			0 AS ''F <3 Unkown Age'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) < 90 or regimenduration IS NULL) and gender = ''F'' THEN 1 ELSE 0 END) AS ''F <3 Months <15'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) < 90 or regimenduration IS NULL) and gender = ''F'' THEN 1 ELSE 0 END) AS ''F <3 Months >15'',
			0 AS ''F 3 -5 Unkown Age'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) >= 90 and Cast(regimenduration AS Decimal) < 180) and gender = ''F'' THEN 1 ELSE 0 END) AS ''F 3 - 5 Months <15'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) >= 90 and Cast(regimenduration AS Decimal) < 180) and gender = ''F'' THEN 1 ELSE 0 END) AS ''F 3 -5 Months >15'',
			0 AS ''F >6 Unkown Age'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) >= 180) and gender = ''F'' THEN 1 ELSE 0 END) AS ''F >6 Months <15'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) >= 180) and gender = ''F'' THEN 1 ELSE 0 END) AS ''F >6 Months >15'',
			0 AS ''M <3 Unkown Age'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) < 90 or regimenduration IS NULL) and gender = ''M'' THEN 1 ELSE 0 END) AS ''M <3 Months <15'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) < 90 or regimenduration IS NULL) and gender = ''M'' THEN 1 ELSE 0 END) AS ''M <3 Months >15'',
			0 AS ''M 3 -5 Unkown Age'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) >= 90 and Cast(regimenduration AS Decimal) < 180) and gender = ''M'' THEN 1 ELSE 0 END) AS ''M 3 - 5 Months <15'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) >= 90 and Cast(regimenduration AS Decimal) < 180) and gender = ''M'' THEN 1 ELSE 0 END) AS ''M 3 -5 Months >15'',
			0 AS ''M >6 Unkown Age'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) >= 180) and gender = ''M'' THEN 1 ELSE 0 END) AS ''M >6 Months <15'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) >= 180) and gender = ''M'' THEN 1 ELSE 0 END) AS ''M >6 Months >15'',
