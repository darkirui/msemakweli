DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spdatimtxcurrdata`(
...
BEGIN
    set @querydropformerrecords = concat('
            ...
            fd.facilityname,fd.mflcode,fd.countyname,fd.subcountyname,fd.regionname,
			...
			Count(*) As Total,
			0 AS ''F <3 Unkown Age'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) < 100 or regimenduration IS NULL) and gender = ''F'' THEN 1 ELSE 0 END) AS ''F <3 Months <15'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) < 100 or regimenduration IS NULL) and gender = ''F'' THEN 1 ELSE 0 END) AS ''F <3 Months >15'',
			0 AS ''F 3 -5 Unkown Age'',
			SUM((CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) >=100  and Cast(regimenduration AS Decimal) < 180) and gender = ''F'') OR  
            (CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) >= 180) and gender = ''F'') THEN 1 ELSE 0 END) AS ''F 3 - 5 Months <15'',
			SUM((CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) >=100  and Cast(regimenduration AS Decimal) < 180) and gender = ''F'') OR
            (CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) >= 180) and gender = ''F'') THEN 1 ELSE 0 END) AS ''F 3 -5 Months >15'',
			0 AS ''F >6 Unkown Age'',
			0 AS ''F >6 Months <15'',
			0 AS ''F >6 Months >15'',
			0 AS ''M <3 Unkown Age'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) < 100 or regimenduration IS NULL) and gender = ''M'' THEN 1 ELSE 0 END) AS ''M <3 Months <15'',
			SUM(CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) < 100 or regimenduration IS NULL) and gender = ''M'' THEN 1 ELSE 0 END) AS ''M <3 Months >15'',
			0 AS ''M 3 -5 Unkown Age'',
			SUM((CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) >=100  and Cast(regimenduration AS Decimal) < 180) and gender = ''M'') OR
            (CASE WHEN Cast(AgeAtLastVisit AS Decimal) < 15 and (Cast(regimenduration AS Decimal) >= 180) and gender = ''M'' THEN 1 ELSE 0 END) THEN 1 ELSE 0 END) AS ''M 3 - 5 Months <15'',
			SUM((CASE WHEN Cast(AgeAtLastVisit AS Decimal) >= 15 and (Cast(regimenduration AS Decimal) >=100  and Cast(regimenduration AS Decimal) < 180) and gender = ''M'' THEN 1 ELSE 0 END) AS ''M 3 -5 Months >15'',
			0 AS ''M >6 Unkown Age'',
			SUM() AS ''M >6 Months <15'',
			SUM() and gender = ''M'' THEN 1 ELSE 0 END) AS ''M >6 Months >15'',
		...
		from(
			...
			from(
				select facilityname,personid from patienttxcurr where voided = 0 and reportingperiod = ''',reportingperiod,'''
			)report left join(
		...
		)alldata left join facilitydetails fd
		on alldata.facilityname = fd.facilityname
        CROSS JOIN (SELECT @row_number := 0) AS rnumber
		group by alldata.facilityname;');
